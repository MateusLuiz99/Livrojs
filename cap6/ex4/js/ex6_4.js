function verSenha(){
    ...
    if(senha.match(/[0-9]/g) == null){
        erros.push("possuir números (no mínino, 1)");
    }

    if(!senha.match(/[a-z]/g)){
        erros.push("possuir letras minúsculas (no mínino, 1)");
    }

    if(!senha.match(/[A-Z]/g) || senha.match(/[A-Z]/g).length == 1){
        erros.push("possuir letras maiúsculas (no mínino, 2)");
    }

    if(!senha.match(/[\W|_]/g)){
        erros.push("possuir símbolos (no mínino, 1)");
    }
